        ...
        <div className="md:hidden px-4 pb-4 space-y-2 text-black dark:text-white">
          ...
          {!isAuthenticated ? (
            <div className="flex gap-2">
              <Link to="/login" onClick={() => setOpen(false)} className="py-2 text-indigo-600">Login</Link>
              <Link to="/register" onClick={() => setOpen(false)} className="py-2 px-3 rounded-md bg-indigo-600 text-white">Register</Link>
            </div>
          ) : (
            <div className="space-y-2">
              <Link to="/watchlist" onClick={() => setOpen(false)} className="block py-2">My Watchlist</Link>
              <button onClick={() => { setOpen(false); navigate('/dashboard'); }} className="w-full text-left py-2">Profile</button>
