...
export default function PostCard({
  ...
  const postAuthorName = (post.author && (post.author.username || post.author.name)) || (postAuthorId ? String(postAuthorId).slice(0, 6) : "Unknown");
  ...
  const handleCreateComment = async (e) => {
    ...
    setPosting(true);
    try {
      const created = await onComment(clubId, postId, { content, spoiler: !!commentSpoiler });
      ...
      const cid = getId(createdComment) || createdComment._id || createdComment.id;
      if (!cid) {
      ...
      if (!createdComment.author) {
        createdComment.author = typeof meId === "object" && meId?.username ? { _id: myId, username: meId.username } : { _id: myId, username: "You" };
  ...
  const handleDeleteComment = async (comment) => {
    ...
    if (!commentId) {
      alert("Comment id missing — cannot delete (backend should return saved comment id).");
      return;
    }
    ...
    try {
      ...
      // remove locally
      setLocalComments((c) => c.filter((cm) => String(getId(cm) || cm._id || cm.id) !== String(commentId)));
    } catch (err) {
  ...
  return (
    <div className="bg-white dark:bg-slate-800 p-4 rounded-xl border border-slate-100 dark:border-slate-700">
      <div className="flex items-start justify-between gap-3">
        <div>
          ...
          <div className="text-sm text-slate-500 dark:text-slate-400 mt-1">
            {postAuthorName} • {new Date(post.createdAt || Date.now()).toLocaleString()}
          </div>
        </div>
        ...
        <div className="flex items-center gap-2">
          {meIsAuthor && (
            <button
              onClick={() => {
                if (typeof onDeletePost === "function") onDeletePost(clubId, postId);
              }}
          ...
          <>
            {!showSpoiler ? (
              <div className="p-3 bg-slate-50 dark:bg-slate-900 rounded-md">
                ...
                <button onClick={() => setShowSpoiler(true)} className="mt-2 px-3 py-1 rounded-md bg-indigo-600 text-white text-sm">
                  Show Spoiler
                </button>
              </div>
            ) : (
      ...
      {/* comments */}
      <div className="mt-4">
        <div className="text-sm font-semibold text-black dark:text-white mb-2">Comments ({localComments?.length ?? 0})</div>

        {(localComments || []).length === 0 ? (
          <div className="text-slate-600 dark:text-slate-400 p-3 rounded-md bg-slate-50 dark:bg-slate-900">No comments yet. Be the first to comment.</div>
        ) : (
          <div className="space-y-3">
            {(localComments || []).map((cm, idx) => {
              const cid = getId(cm) || cm._id || cm.id || `local-${idx}`;
              const authorId = typeof cm.author === "object" ? getId(cm.author) : cm.author;
              const displayName = (cm.author && (cm.author.username || cm.author.name)) || (authorId ? String(authorId).slice(0, 6) : "User");
              const canDelete = Boolean(authorId) && String(authorId) === String(myId);
              return (
                <div key={cid} className="p-3 bg-slate-50 dark:bg-slate-900 rounded-md border border-slate-100 dark:border-slate-700">
                  <div className="flex items-start justify-between gap-3">
                    <div className="text-sm">
                      <div className="font-medium text-black dark:text-white">{displayName}</div>
                      <div className="text-xs text-slate-500 dark:text-slate-400">{new Date(cm.createdAt || Date.now()).toLocaleString()}</div>
                    </div>
                  ...
                  <div className="mt-2 text-slate-700 dark:text-slate-300">
                    {cm.spoiler ? (
                      ...
                      <div className="whitespace-pre-line">{cm.content ?? cm.body ?? ""}</div>
                    )}
                  </div>
                </div>
              );
      ...
      {/* comment form */}
      <form onSubmit={handleCreateComment} className="mt-4 bg-slate-50 dark:bg-slate-900 p-3 rounded-md border border-slate-100 dark:border-slate-700">
        <textarea
          value={commentText}
        ...
        <div className="mt-2 flex items-center justify-between">
          ...
          <div className="flex gap-2">
            <button type="button" onClick={() => { setCommentText(""); setCommentSpoiler(false); }} className="px-3 py-1 rounded-md border text-black dark:text-white">Cancel</button>
            <button type="submit" disabled={posting} className="px-4 py-1 rounded-xl bg-indigo-600 text-white">
...
function SpoilerText({ text = "" }) {
  ...
  return visible ? (
    ...
    <div className="p-2 bg-slate-100 dark:bg-slate-900 rounded">
      ...
      <button onClick={() => setVisible(true)} className="mt-2 px-3 py-1 rounded-md bg-indigo-600 text-white text-sm">Show</button>
    </div>
  );
}
